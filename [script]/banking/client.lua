...
local atms = {
  ...
  {name="ATM", id=277, x=527.26, y=-160.38, z=57.09},
    ...
    while true do
      ...
      if(IsNearBank() or IsNearATM()) then
        if (atBank == false) then
          TriggerEvent('notification:displayhelptext',"Appuyez sur ~INPUT_CONTEXT~ pour gerer votre argent");
        end
        atBank = true
        if IsControlJustPressed(1, 51)  then -- IF INPUT_PICKUP Is pressed
          if (IsInVehicle()) then
      TriggerEvent('notification:notification',"<b>~y~Tu ne peux pas gerer ton argent en voiture</b>");
          else
            if bankOpen then
              closeGui()
              bankOpen = false
...
AddEventHandler('bank:deposit', function(amount)
  if(IsNearBank() == true or depositAtATM == true and IsNearATM() == true or depositAnywhere == true ) then
    if (IsInVehicle()) then
      TriggerEvent('notification:notification',"<b>~y~Tu ne peux pas utiliser un ATM en voiture</b>");
    else
		if type(tonumber(amount)) == "number" and tonumber(amount) > 0 then
			TriggerServerEvent("bank:deposit", tonumber(amount))
		else
			TriggerEvent('notification:notification', "<b>~r~Mauvaise entrée</b>");
		end
    end
  else
    TriggerEvent('notification:notification',"<b>~y~Depot d'argent uniquement en banque</b>");
  end
end)
...
AddEventHandler('bank:withdraw', function(amount)
  if(IsNearATM() == true or IsNearBank() == true or withdrawAnywhere == true) then
    ...
    TriggerEvent('notification:notification',"<b>~y~Ce n'est pas une banque ou un ATM</b>");
  end
end)
...
AddEventHandler('bank:givecash', function(toPlayer, amount)
	...
	DisplayOnscreenKeyboard(1, "Quantité :", "", "", "", "", "", 8)
        ...
        if (GetOnscreenKeyboardResult()) then
			...
			if type(tonumber(res)) == "number" and res > 0 then
		...
		TriggerEvent('notification:notification',"<b>~b~Pas de joueur proche</b>");
	end
end)
...
AddEventHandler('bank:transfer', function(fromPlayer, toPlayer, amount)
  ...
  if (playing ~= false) then
    ...
    TriggerEvent('notification:notification',"<b>~y~ Attention ! Ce joueur n'est pas en ligne</b>");
  end
end)
